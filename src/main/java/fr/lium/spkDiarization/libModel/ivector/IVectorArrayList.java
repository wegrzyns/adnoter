 ...
 */
public class IVectorArrayList extends ArrayList<fr.lium.spkDiarization.libModel.ivector.IVector> {
	 ...
	 * @param arg0 the arg0
	 */
	public IVectorArrayList(Collection<? extends fr.lium.spkDiarization.libModel.ivector.IVector> arg0) {
		super(arg0);
	}
	...
	public static IVectorArrayList loadIVector(String fileName) throws IOException, DiarizationException {
		...
		for (String line : list) {
			...
			int i = -1;
			fr.lium.spkDiarization.libModel.ivector.IVector iv = new fr.lium.spkDiarization.libModel.ivector.IVector(words.length - 1);
			if (len == -1) {
				len = words.length;
			} else {
				if (len != words.length) {
