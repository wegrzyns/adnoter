...
//		firstOrderStatisticCopy.ensureCapacity(minCapacity);
	 ...
	 * @throws DiarizationException the diarization exception
	 */
	protected void estimateTotalVariabilityMatrix(ArrayList<MatrixSymmetric> _l_h_inv, fr.lium.spkDiarization.libModel.ivector.IVectorArrayList iVectorList) throws DiarizationException {
		...
		for (int g = 0; g < nbComponent; g++) {
			...
			for (int j = 0; j < zeroOrderStatistic.size(); j++) {
				...
				fr.lium.spkDiarization.libModel.ivector.IVector iVector = iVectorList.get(j);
...
//						R.add(k, firstOrderStatistic.get(j).get(gi) * iVectorList.get(j).get(k));// ok
	 ...
	 * Estimate i vector.
	 */
	protected fr.lium.spkDiarization.libModel.ivector.IVectorArrayList estimateIVector(ArrayList<MatrixSymmetric> listOfLInvert) {
		// matrice U, occ, order1, L, superCov
		fr.lium.spkDiarization.libModel.ivector.IVectorArrayList iVectorList = new fr.lium.spkDiarization.libModel.ivector.IVectorArrayList();
		...
		for (int sent = 0; sent < zeroOrderStatistic.size(); sent++) {
			...
			if (accumulatorClusterName.size() == 0) {
				iVectorList.add(new fr.lium.spkDiarization.libModel.ivector.IVector(W, "sent_" + sent, Cluster.genderStrings[0]));
			} else {
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public fr.lium.spkDiarization.libModel.ivector.IVectorArrayList trainIVector() throws DiarizationException, IOException {
		...
		fr.lium.spkDiarization.libModel.ivector.IVectorArrayList iVectorList = estimateIVector(_l_h_inv);
		return iVectorList;
	}
